Feature: Get a double espresso as a reward from barista
  ...
  Scenario: Get a double espresso from the barista
    ...
    When the user waits long enough for the description to contain the text "<conversation>"
    Then my hipster bag should contain "a can of beer"
    And the user should go back to the cafe
    And wait in the cafe until they hear the conversation the barista is having on the phone about "<conversation>"
